...
def random_rotation_matrix(strength=None, dtype=None):
    """Generates a random rotation matrix that does not rotate around the x-axis
    ...
    r = np.sqrt(z)
    # Adjust V to be in y-z plane
    V = np.array([0, np.cos(phi) * r, np.sin(phi) * r])
    ...
    rand_R = (np.outer(V, V) - np.eye(3)).dot(Ry)
    return rand_R.astype(dtype)
...
def rasterize_points(points, voxel_size, particle_radius):
    #将点云转换成光栅化的表示形式
    if not (voxel_size > 2 * particle_radius): # 确保每个点能至少被一个体素覆盖
        raise ValueError(
            "voxel_size > 2*particle_radius is not true. {} > 2*{}".format(
                voxel_size, particle_radius))

    points_min = (points - particle_radius).min(axis=0) #点云边界最小点
    points_max = (points + particle_radius).max(axis=0) #点云边界最大点

    arr_min = np.floor_divide(points_min, voxel_size).astype(np.int32) #体素数组最小索引
    arr_max = np.floor_divide(points_max, voxel_size).astype(np.int32) + 1 #体素数组最大索引

    arr_size = arr_max - arr_min #体素数组大小

    arr = np.zeros(arr_size)

    offsets = [] # 创建一个列表 offsets，包含了围绕每个点的八个角的偏移量。
    ...
    # 遍历每个点和它的偏移量，将相应的体素位置设置为1，表示该体素被点云中的点占据。
    return arr_min, voxel_size, arr # 函数返回体素数组的最小索引、体素大小和体素数组本身。
...
def find_valid_fluid_start_positions(box_rasterized, fluid_rasterized):
    ...
    last_pos = box_shape - fluid_shape # [53, -39, 49]
    ...
    for idx in itertools.product(range(0, last_pos[0] + 1),
        ...
        # 如果检测到合适的位置，将该位置标记为有效。

        # last_pos[2]为负，无法进入循环
    ...
    valid_pos = np.stack(np.nonzero(valid_fluid_start_positions_arr), axis=-1) # valid_pos=[]没有合适的位置
...
def create_fluid_data(output_dir, seed, options):
    ...
    #                                         'Fluid*.obj')))
    bounding_boxes = [os.path.join(SCRIPT_DIR, 'models', 'box_bunny.obj')]
    fluid_shapes = [os.path.join(SCRIPT_DIR, 'models', 'bunny.obj')]
    ...
    def create_fluid_object():
        ...
        # fluid = fluid @ R # 施加随机旋转 @ = numpy.dot（）
        ...
        # fluid_vel[:, 2] = np.random.uniform(-max_vel, max_vel) #随机初速度
    ...
    for create_scene_i in range(100):
        ...
        bb_rasterized = rasterize_points(np.concatenate([bb_vol, bb], axis=0),
                                         ...
                                         PARTICLE_RADIUS) #函数返回体素数组的最小索引、体素大小和体素数组本身
