...
def main():
    ...
    all_scenes = sorted(glob(os.path.join(args.input, '*')))
    # for scene in all_scenes:
    #     if os.path.isdir(scene) and 'sim_00' in scene and scene.split('sim_00')[1].lstrip('0')) >= 16
    scene_dirs = [
        scene for scene in all_scenes
        if os.path.isdir(scene) and 'sim_00' in scene
        and int(scene.split('sim_00')[1]) >= 54
    ]
    print(scene_dirs)
