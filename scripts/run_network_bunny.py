...
def run_sim_tf(trainscript_module, weights_path, scene, num_steps, output_dir,
    ...
    fluids = []
    x = scene['fluids']
    points1 = obj_volume_to_particles(x['path'])[0]
    points1 += np.asarray([x['translation']], dtype=np.float32)

    x2 = scene['fluids2']
    points2 = obj_volume_to_particles(x2['path'])[0]
    points2 += np.asarray([x2['translation']], dtype=np.float32)

    points = torch.cat([points1, points2], dim=0)
    velocities = np.empty_like(points)
    velocities[:, 0] = x2['velocity'][0]
    velocities[:, 1] = x2['velocity'][1]
    velocities[:, 2] = x2['velocity'][2]
...
def run_sim_torch(trainscript_module, weights_path, scene, num_steps,
    ...
    velocities[:, 2] = x2['velocity'][2]
    range_ = range(x['start'], x['stop'], x['step'])
    fluids.append((points.astype(np.float32), velocities.astype(np.float32), range_))
