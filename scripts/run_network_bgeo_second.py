...
dataset_dir = "../DeepLagrangianFluids-master/datasets/dpi_dam_break"
class PhysicsSimDataFlow(dataflow.RNGDataFlow):
    ...
    def __iter__(self):
        # iter(dataset)的时候调用，返回一个带有下两帧的数据
...
def run_sim_torch(trainscript_module, weights_path, scene, num_steps,
    ...
    # model.load_state_dict(weights) #如果用的是pretrained_model就不用.get("model")
...
def main():
    ...
    for data in val_dataset:
        # liquid3d
        # if data['scene_id0'][0] == 'sim_0210' and data['frame_id0'][0] == 0:
        #dambreak
        if data['scene_id0'][0] == '0' and data['frame_id0'][0] == 0:
        # if data['frame_id0'][0] % 50 == 0:
            frame = data
