...
def main():
    ...
    while trainer.keep_training(step,
        ...
        if (trainer.current_step) % (0.5 * _k) == 0:
            for k, v in evaluate_torch(model,
                ...
                if(k == "err_n1" and v < min_err):
                    min_err = v
                    torch.save({'model': model.state_dict()}, str(step.item())+'_weights_dambreak_best.pt')
                    print("=================update best model: err_n1=" + str(v) + "===============")
            # for k, v in evaluate_whole_sequence_torch(model,
            #                      val_dataset,
            #                      frame_skip=20,
            #                      device=device,
            #                      **cfg.get('evaluation', {})).items():
            #     trainer.summary_writer.add_scalar('eval/' + k, v,
            #                                       trainer.current_step)
            #     if(k == "whole_seq_err" and v < min_err):
            #         min_err = v
            #         torch.save({'model': model.state_dict()}, str(step.item())+'_weights_dambreak_best.pt')
            #         print("=================update best model: err_n1=" + str(v) + "===============")
        # 第1000次迭代eval一次
